...
#vlc also works
#(YTFZF_PLAYER)
video_player="mpv --fs --sub-auto=all --slang=en"

#the player to use when choosing a video format with $YTFZF_PREF
#(YTFZF_PLAYER_FORMAT)
video_player_format="mpv --fs --sub-auto=all --slang=en --ytdl-format="

#the player to use for audio ( option -m )
#(YTFZF_AUDIO_PLAYER)


#rnd_col="#$(xxd -u -l 3 -p /dev/urandom)"

audio_player="mpv --no-video --player-operation-mode=pseudo-gui --window-minimized=yes --background=#$(xxd -u -l 3 -p /dev/urandom) --keepaspect-window=no"
...
#same as --trending=
trending_tab="music, gaming, movies, technology, news"
...
subscriptions_file=$HOME/.config/ytfzf/subscriptions
...
thumbnail_video_info_text () {
         printf "\n ${c_cyan}%s" "$title"
         printf "\n ${c_cyan}Channel      ${c_green}%s" "$channel"
         printf "\n ${c_cyan}Duration     ${c_yellow}%s" "$duration"
         printf "\n ${c_cyan}Views        ${c_magenta}%s" "$views"
         printf "\n ${c_cyan}Date         ${c_cyan}%s" "$date"
